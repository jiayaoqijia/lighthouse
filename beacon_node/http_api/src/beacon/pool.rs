...
use types::{
    ...
    SignedBlsToExecutionChange, SignedInclusionList, SignedVoluntaryExit, SingleAttestation,
            ...
            |task_spawner: TaskSpawner<T::EthSpec>,
             ...
             network_tx: UnboundedSender<NetworkMessage<T::EthSpec>>| {
                task_spawner.blocking_json_task(Priority::P0, move || {
                    ...
                    let current_slot = chain.slot_clock.now().ok_or_else(|| {
                    ...
                    let slot_start = chain.slot_clock.start_of(slot).ok_or_else(|| {
                        warp_utils::reject::custom_server_error(
                            "Unable to determine slot start time".to_string(),
                        )
                    })?;

                    let now = chain.slot_clock.now_duration().ok_or_else(|| {
                        warp_utils::reject::custom_server_error(
